...
class UpdateProduct extends React.Component {
    ...
    componentWillReceiveProps (nextProps) {
      const { name, description, category, subCategory, gender, color, imageUrl, price, stocks } = nextProps.product;
      this.setState({name: name, description: description, category: category, subCategory: subCategory, gender: gender, color: color, imageUrl: imageUrl, price: price, stocks: stocks});
      this.setState({displayImage: imageUrl[0]});
    }

    componentWillMount () {
      const { user } = this.props.auth;
      if (!user || !user.isAdmin) browserHistory.push('/products');
      else this.props.fetchSingleProductFromServer(this.props.params.pid);
    }
    ...
    handleSubmitImage (event) {
      ...
      this.props.adminUploadFilesToServer(data)
        .then(filename => this.setState({ imageUrl: [...this.state.imageUrl, filename ] }))
        .catch(err => console.error(err))
    }

    handleCancelImage (evt, img, idx) {
      evt.preventDefault();
      this.setState({ canceledImages: [...this.state.canceledImages, img] });
      const newImages = [ ...this.state.imageUrl.slice(0, idx), ...this.state.imageUrl.slice(idx + 1)];
      this.setState({ imageUrl: newImages });
    }

    handleUpdateProduct (evt) {
      evt.preventDefault();
      // console.log(this.state.canceledImages);
      this.state.canceledImages.map(img => this.props.adminRemoveImage(img));
      this.props.adminUpdateProduct(this.props.product.id, this.state);
    }

    handleRemoveProduct (evt) {
      evt.preventDefault();
      this.props.adminRemoveProduct(this.props.product.id);
    }
        ...
        <section className="col-xs-offset-1 col-xs-10" style={styles.section}>
          ...
          <div className="row">
            <div className="col-lg-6 col-sm-12">
              <div className="row">
                <div className="col-xs-12" style={styles.subheading}>EDIT PRODUCT</div>
              </div>
              ...
              <div className="row center-xs">
                <div className="col-xs-6">
                    <button id="upload" type="button" onClick={this.handleUpdateProduct}>Update</button>
                    <button id="upload" type="button" onClick={this.handleRemoveProduct}>Delete</button>
                </div>
              </div>
            </div>
...
const mapStateToProps = ({ admin, product, auth }) => ({ admin, product, auth });
